...
from src.runner.predictors import AcdcVSRPredictor


class AcdcVSRRefinePredictor(AcdcVSRPredictor):
        ...
        for batch in trange:
            ...
            with torch.no_grad():
                ...
                if self.exported:
                    ...
                    outputs = [self._denormalize(output) for output in outputs[-1]]
    ...
    def _compute_losses(self, outputs, targets):
        ...
        for loss_fn in self.loss_fns:
            losses.append(torch.stack([loss_fn(output, target) for output, target in zip(outputs[-1], targets)]))
