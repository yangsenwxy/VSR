...
class AcdcVSRRefineNetPredictor(BasePredictor):
    ...
    def predict(self):
        ...
        for batch in trange:
            ...
            inputs, targets, index, all_imgs, fs, fe = self._get_inputs_targets(batch)
            T = len(inputs)
            with torch.no_grad():
                outputs = self.net(inputs, all_imgs, fs, fe)
    ...
    def _get_inputs_targets(self, batch):
        ...
        """
        return batch['lr_imgs'], batch['hr_imgs'], batch['index'], batch['all_imgs'], batch['frame_start'], batch['frame_end']
