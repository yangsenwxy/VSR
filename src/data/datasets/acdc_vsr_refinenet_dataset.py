...
class AcdcVSRRefineDataset(BaseDataset):
    ...
    """
    def __init__(self, downscale_factor, transforms, augments=None, num_frames=5, temporal_order='last', pos_code_path=None, **kwargs):
    ...
    def __getitem__(self, index):
        ...
        if self.pos_code_path is None:
            ...
            with open(self.pos_code_path, 'rb') as f:
                ...
                pos_codes = pos_codes[patient]
        pos_codes = self.transforms(pos_codes, normalize_tags=[False])
        ...
        pos_codes = pos_codes.repeat(35 // num_all_frames + 2)[:35].unsqueeze(1)
        
        return {'hr_imgs': hr_imgs, 'all_imgs': all_imgs, 'pos_codes': pos_codes, 'start_frame': start, 'num_all_frames': num_all_frames, 'index': index}