...
class AcdcVSRRefineLogger(BaseLogger):
    ...
    def _add_images(self, epoch, train_batch, train_outputs, valid_batch, valid_outputs):
        ...
        train_sr_img = make_grid(train_outputs[-1][-1], nrow=1, normalize=True, scale_each=True, pad_value=1)
        valid_hr_img = make_grid(valid_batch['hr_imgs'][-1], nrow=1, normalize=True, scale_each=True, pad_value=1)
        valid_sr_img = make_grid(valid_outputs[-1][-1], nrow=1, normalize=True, scale_each=True, pad_value=1)
