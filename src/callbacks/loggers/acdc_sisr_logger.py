...
class AcdcSISRLogger(BaseLogger):
    """The ACDC logger for the SISR task.
    """
    ...
    def _add_images(self, epoch, train_batch, train_outputs, valid_batch, valid_outputs):
        ...
        train_sr_img = make_grid(train_outputs, nrow=1, normalize=True, scale_each=True, pad_value=1)
        valid_hr_img = make_grid(valid_batch['hr_img'], nrow=1, normalize=True, scale_each=True, pad_value=1)
        valid_sr_img = make_grid(valid_outputs, nrow=1, normalize=True, scale_each=True, pad_value=1)
