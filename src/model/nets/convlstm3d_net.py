...
class ConvLSTM3DNet(BaseNet):
    ...
    def forward(self, inputs):
        ...
        for i in range(num_frames):
            ...
            if i == 0:
                self.lstm_block._init_hidden(in_feature.size(0), in_feature.size(2), \
                                             in_feature.size(3), in_feature.size(4))
...
class _InBlock(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.add_module('conv1', nn.Conv3d(in_channels, 4 * out_channels, kernel_size=3, padding=1))
        self.add_module('prelu1', nn.PReLU(num_parameters=1, init=0.2))
        self.add_module('conv2', nn.Conv3d(4 * out_channels, out_channels, kernel_size=1))
    ...
    def __init__(self, in_channels, out_channels, upscale_factor):
        ...
        if (math.log(upscale_factor, 2) % 1) == 0:
            for i in range(int(math.log(upscale_factor, 2))):
                self.add_module(f'conv{i+1}', nn.Conv3d(in_channels, 4 * in_channels, kernel_size=3, padding=1))
                self.add_module(f'pixelshuffle{i+1}', PixelShuffle3d(2))
            self.add_module(f'conv{i+2}', nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1))
        elif upscale_factor == 3:
            self.add_module('conv1', nn.Conv3d(in_channels, 9 * in_channels, kernel_size=3, padding=1))
            self.add_module('pixelshuffle1', PixelShuffle3d(3))
...
class ConvLSTMCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, bias, memory):
        ...
        if memory:
            ...
            self.conv = nn.Conv3d(in_channels=self.input_dim * 2,
                                  out_channels=4 * self.hidden_dim,
                                  kernel_size=self.kernel_size,
                                  padding=self.padding,
                                  bias=self.bias)
    ...
    def init_hidden(self, batch_size, depth, height, width):
        return (Variable(torch.zeros(batch_size, self.hidden_dim, depth, height, width)).cuda(),
                Variable(torch.zeros(batch_size, self.hidden_dim, depth, height, width)).cuda())
...
class _ConvLSTM(nn.Module):
    ...
    def _init_hidden(self, batch_size, depth, height, width):
        init_states = []
        for i in range(self.num_layers):
            init_states.append(self.cell_list[i].init_hidden(batch_size, depth, height, width))
...
class PixelShuffle3d(nn.Module):
    ...
    def forward(self, input):
        ...
        input_view = input.contiguous().view(n, out_c, r, r, d, h, w)
        output = input_view.permute(0, 1, 4, 5, 2, 6, 3).contiguous().view(n, out_c, out_d, out_h, out_w)
        return output